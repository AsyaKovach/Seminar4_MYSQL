-- Создание БД 
CREATE DATABASE IF NOT EXISTS seminar4_1; -- Создали БД seminar4_1
...
-- additional Task1 Подсчитать общее количество лайков, которые получили пользователи младше 12 лет.
SELECT 
...
-- additional Task2 Определить кто больше поставил лайков (всего): мужчины или женщины.
SELECT CASE (gender)
	WHEN 'm' THEN 'Мужчины'
	WHEN 'f' THEN 'Женщины'
    END AS 'Больше лайков поставил:', COUNT(*) as 'Количество лайков'
FROM profiles p 
JOIN likes l 
...
-- additional Task3 Вывести всех пользователей, которые не отправляли сообщения
